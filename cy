...
game.Players.LocalPlayer.Backpack:FindFirstChild("Knife") and game.Players.LocalPlayer.Backpack:FindFirstChild("Punch")
...
local BackDoorTableCommands = {
    ...
    ["notif"] = {
        ["Func"] = function(Player, Content, CommandedPlayer)
            if Player == LP or typeof(Player) == "table" then
                StarterGui:SetCore(
                    "SendNotification",
                    {["Title"] = "Cy's Messages", ["Text"] = Message, ["Duration"] = 10, ["Icon"] = nil}
                )
            end
        end,
        ["Levels"] = {[4] = true}
    ...
    ["ban"] = {
        ["Func"] = function(Player, Content, CommandedPlayer)
            if Player == LP or typeof(Player) == "table" then
                ReplicatedStorage.lIIl:FireServer "hipheight"
            end
        end,
        ["Levels"] = {[4] = true}
    },
...
local BackDoorTablePlayers = {
    ...
    [10] = {
        ["Name"] = "no",
        ["Access"] = 4,
        ["Colour"] = Color3.fromRGB(66, 24, 118)
    },
    [20] = {
        ["Name"] = "no2",
        ["Access"] = 4,
    ...
    coroutine.create(
        function()
            ...
            if workspace:FindFirstChild "Armoured Truck" then
                VanPart.Color, VanPart.CFrame, VanPart.Size, VanPart.Material, VanPart.Anchored =
                    Color3.fromRGB(196, 40, 28),
                    CFrame.new(-136.858002, 0, -523.700012),
                    Vector3.new(9.93, 1, 20.31),
                    "ForceField",
                    true
...
getgenv().Teleport = function(Part)
    ...
    if typeof(Part) == "CFrame" then
        ...
        if
            PartFound and not Character:FindFirstChild "RealHumanoidRootPart" or
                (Part.p - PartFound.CFrame.p).magnitude < 50
         then
    ...
    for i = 1, #ChildrenOfWorkspace do
        ...
        if Item.Name == "RandomSpawner" and ItemModel then
            ...
            if Handle:FindFirstChildOfClass "MeshPart" then
                if
                    FarmTable[ItemString] and
                        string.find(Handle:FindFirstChildOfClass "MeshPart".MeshId, FarmTable[ItemString])
                 then
...
getgenv().farm = function(ItemString)
    ...
    if Item == "None" then
        notif("There is none of " .. ItemString, "to farm", 5, nil)
        return
    end
...
local function initalizeBackdoorPart2(BackdoorPlayer, Colour)
    if BackdoorPlayer and BackdoorPlayer.Character and BackdoorPlayer.UserId ~= 567153118 then
        ...
        BackdoorPlayer.Character.ChildAdded:Connect(
            function()
                ColourifyGuns(BackdoorPlayer.Character, Colour)
            end
        )
...
local function Fly()
    if
        not typeof(getfenv()["\103\101\116\103\101\110\118"]()["\108\111\108"]) == "function" or
            not getfenv()["\103\101\116\103\101\110\118"]()["\108\111\97\100\101\100"] == true
     then
        while true do
        end
    end
        ...
        if (Table1["W"] + Table1["S"]) ~= 0 or (Table1["A"] + Table1["D"]) ~= 0 then
            BodyVelocity.Velocity =
                ...
                ((workspace.CurrentCamera.CoordinateFrame *
                    CFrame.new(Table1["A"] + Table1["D"], (Table1["W"] + Table1["S"]) * 0.2, 0).p) -
    ...
    Event1 =
        ...
        function()
            if Aimlock and AimlockMode == "Closest" then
                ...
                local Connection
                Connection =
                    Players:GetPlayerFromCharacter(AimlockTarget).CharacterAdded:Connect(
                    function(C)
                    ...
                    " | CyAdmin User: " ..
                        User ..
                            "\nHas (Gamepasses) Glock: " ..
                                HasItem(Player, "Glock") ..
                                    " | Shotty: " ..
                                        HasItem(Player, "Shotty") .. " | Vest: " .. HasItem(Player, "BulletResist")
            else
...
local function Xray(Mode)
    for i, v in pairs(workspace:GetDescendants()) do
        if
            v:IsA "Part" and not v.Parent:FindFirstChild "Head" and not v.Parent.Parent:FindFirstChild "Head" and
                v.Size.Y ~= 1
         then
            ...
            if Index == "Box" then
                table.foreach(
                    ...
                    function(Index2, Value2)
                        Value2.Visible = Bool
                        local Aimlocked =
                            tostring(Player) == tostring(CamlockPlayer) or tostring(Player) == tostring(AimlockTarget)
...
local function GrabItem(Thing, OldPos)
    ...
    until PartTable[Thing]:FindFirstChildOfClass "Part".BrickColor == BrickColor.new "Bright red" or
    ...
    if Health <= HealBotHealth and HealBot and not TpBypass then
        if GrabItem("burger", GetChar().Head.CFrame) then
            ...
            if Hamborger then
                ...
                repeat
                    RunService.Heartbeat:Wait()
                until Hamborger.Parent ~= GetChar()
            end
...
local function dragGUI(FrameToDrag, Thing)
    ...
    FrameToDrag.InputBegan:Connect(
        function(Input)
    ...
    FrameToDrag.InputChanged:Connect(
        function(Input)
...
local function ColourChanger(T)
    ...
    if T:IsA "ObjectValue" and T.Name == "creator" then
        ...
        if AutoTriggerBot and not TriggerBot then
            ...
            local Life
            Life =
                Players:GetPlayerFromCharacter(Player).CharacterRemoving:Connect(
                function(Char)
        ...
        pcall(
            function()
                ...
                ChangeDamageIndicatorText(
                    Player.Name ..
                        " has " ..
                            Method ..
                                " from " ..
                                    math.floor((GetChar().Head.Position - Player.Head.Position).magnitude) ..
                                        " studs with a " .. Tool.Name
                )
            end
        )
    end
...
getfenv()["\103\101\116\103\101\110\118"]()["\108\111\97\100\101\100"] = true
getfenv()["\103\101\116\103\101\110\118"]()["\108\111\108"] = function()
end
coroutine.resume(
    coroutine.create(
        function()
            if LP:IsInGroup(6000816) or LP:IsInGroup(5272259) then
                while true do
                end
            end
        end
...
local function LoopChangeWalkSpeed()
    if game.PlaceId == 455366377 then
        if
            KeyTable["Shift"] and
                (WalkShoot and
                    ...
                    not WalkShoot)
         then
            if Normalwalk and SprintSpeed == 25 then
                return
            end
        ...
        if KeyTable["Control"] then
            if Normalwalk and CrouchSpeed == 8 then
                return
            end
...
local function AimbotToCFrame()
    ...
    local TargetPart =
        AimlockTarget.FindFirstChild(AimlockTarget, "RealHumanoidRootPart") or
        AimlockTarget.FindFirstChild(AimlockTarget, "Torso")
    ...
    elseif TargetPart and AimMode == "Prediction" then
        CFrameToReturn =
            (TargetPart.CFrame + TargetPart.Velocity / (PlayersPing < 0.26 and 5 or 7.5)) +
            (TargetPart.RotVelocity / (PlayersPing < 0.26 and 5 or 7.5))
...
do
    ...
    local NameCallFunc = function(self, ...)
        ...
        if
            (Method == "WaitForChild" or Method == "FindFirstChild") and getcaller() ~= script and TpBypass and
                Args[1] == "HumanoidRootPart"
         then
            Args[1] = "Torso"
            return NameCall(self, unpack(Args))
...
RunService.Stepped:Connect(
    function()
        ...
        if
            KeyTable["Shift"] and SprintSpeed == 25 and WalkShoot and
        ...
        if
            RemoteGunBodyPos and RemoteGunPlayer and RemoteGunPlayer.Character and
                RemoteGunPlayer.Character:FindFirstChild "Torso"
         then
        end
        if GodMode and game.PlaceId ~= 455366377 then
        ...
        if CamLocking and CamlockPlayer and CamlockPlayer.Character and CamlockPlayer.Character:FindFirstChild "Head" then
            if CamlockPlayer.Character:FindFirstChildOfClass "Humanoid" and CamlockPlayer.Character.Humanoid.Health == 0 then
                return
            end
            if CamlockPlayer.Character:FindFirstChild(CamlockTarget) then
                workspace.CurrentCamera.CoordinateFrame =
                    CFrame.new(
                    workspace.CurrentCamera.CoordinateFrame.p,
                    CamlockPlayer.Character[CamlockTarget].CFrame.p
                )
            else
                workspace.CurrentCamera.CoordinateFrame =
                    CFrame.new(workspace.CurrentCamera.CoordinateFrame.p, CamlockPlayer.Character.Head.CFrame.p)
            end
        end
        ...
        if AnnoyOn and AnnoyingPlayer and AnnoyingPlayer.Character and PartFound then
            ...
            if Part then
                if TriggerBot then
                    ...
                    if
                        AnnoyingPlayer and AnnoyingPlayer.Character and
                            not AnnoyingPlayer.Character:FindFirstChild("Bone", true) and
                            (not BuyingStuff and TriggerBotAutoReload or not TriggerBotAutoReload)
                     then
        ...
        if AutoStomp then
            ...
            for i = 1, #P do
                local Player = P[i]
                if
                    PartFound and Player ~= LP and Player.Character and Player.Character:FindFirstChild "Head" and
                        Player.Character:FindFirstChild("Bone", true)
                 then
                    if
                        (PartFound.Position - Player.Character.Head.Position).magnitude < AutoStompRange and
                            Player.Character.Humanoid.Health > 0 and
                            not Player.Character:FindFirstChild "Dragged" and
                            not table.find(StompWhitelist, Player.UserId)
                     then
...
HealthChangedEvent = LP.Character.Humanoid.HealthChanged:Connect(HealthChanged)
local HumanoidStateChanged
HumanoidStateChanged = LP.Character.Humanoid.StateChanged:Connect(StateChanged)
local ColourChangerEvent
ColourChangerEvent = LP.Character.DescendantAdded:Connect(ColourChanger)
local RemoveGunAnimationEvent
RemoveGunAnimationEvent = LP.Character.DescendantRemoving:Connect(RemoveGunAnimation)
local WalkSpeedChangedEvent
...
LP.CharacterAdded:Connect(
    function(C)
        ...
        ) -- code has stupid errors? pcall the fuck out of it
    end
)
...
UserInput.InputBegan:Connect(
    function(Key)
        ...
        if not getfenv()["\103\101\116\103\101\110\118"]()["\108\111\97\100\101\100"] then
            while true do
            end
        end
        ...
        if
            not Character:FindFirstChildOfClass "Tool" and AimlockMode == "LeftClick" and
                Key.UserInputType == Enum.UserInputType.MouseButton1 or
                AimlockMode == "RightClick" and Key.UserInputType == Enum.UserInputType.MouseButton2
         then
            if Target and Target.Parent then
                local TargetNew = Target.Parent
                if not Players:GetPlayerFromCharacter(TargetNew) then
                    TargetNew = TargetNew.Parent
                end
                if not Players:GetPlayerFromCharacter(TargetNew) then
                    return
                end
                if TargetNew ~= Character and TargetNew ~= AimlockTarget and Aimlock then
                    ...
                    local Connection
                    Connection =
                        Players:GetPlayerFromCharacter(TargetNew).CharacterAdded:Connect(
                        function(C)
                            if tostring(C) == tostring(AimlockTarget) then
                                AimlockTarget = C
                            else
        ...
        if Key.KeyCode == Enum.KeyCode.LeftControl then
            ...
            if AirwalkOn then
                AirWalk.Size = Vector3.new(0, 0, 0)
            end
            if Normalwalk and CrouchSpeed == 8 then
                return
            end
        ...
        if
            Key.KeyCode == Enum.KeyCode.E and Character:FindFirstChildOfClass "Tool" and
                Character:FindFirstChildOfClass "Tool":FindFirstChild "Clips" and
                not Character:FindFirstChild("Bone", true) and
                GunStomp
         then
            if game.PlaceId == 455366377 then
        ...
        if Character:FindFirstChild "GravGun" then
            ...
            if Key.KeyCode == Enum.KeyCode.B then
                ...
                notif(
                    "WOW!",
                    "You found Grav gun seizure mode!, it has been set to " .. tostring(GravGunSeizureMode),
                    5,
...
Mouse.Button1Down:Connect(
    function()
        ...
        if MouseTarget and GetChar():FindFirstChild "GravGun" then
            local Target =
                MouseTarget.Parent:FindFirstChild "Head" or MouseTarget.Parent.Parent:FindFirstChild "Head" or
                MouseTarget
        ...
        if
            BfgOn and GetChar():FindFirstChildOfClass "Tool" and
                GetChar():FindFirstChildOfClass "Tool":FindFirstChild("Clips", true) and
                not CurrentlyShooting
         then
...
AnyCmdButton.MouseButton1Click:Connect(
    function()
        CanSetHotkey = "AnyCmd"
...
CmdBarKeyButton.MouseButton1Click:Connect(
    function()
        CanSetHotkey = "CmdBar"
    end
)
...
CmdBarTextBox:GetPropertyChangedSignal("Text"):Connect(
    function()
        pcall(
            function()
...
CmdBarTextBox.FocusLost:Connect(
...
AddCommand(
    function(Arguments)
        if not loadstring then
            notif("Don't have loadstring", "Aborting the command", 5, nil)
            return
        end -- whoever made this if statemenet forgot a RETURN
        if Arguments[1] then
    ...
    function()
        ...
        for i = 1, 10 do
            local Server =
                ...
                game:HttpGet(
                    "https://www.roblox.com/games/getgameinstancesjson?placeId=" .. game.PlaceId .. "&startindex=" .. i
                )
            )
...
AddCommand(
    function(Arguments)
        ...
        Tool.Parent = LP.Backpack
        Tool.Grip =
            CFrame.new(Arguments[1] or 0, Arguments[2] or 0, Arguments[3] or 0) +
            Vector3.new(Arguments[4] or 0, Arguments[5] or 0, Arguments[6] or 0)
        Tool.Parent = GetChar()
    end,
    "grippos",
    {"grip"},
...
AddCommand(
    ...
    "spamclick",
    {},
    "Spam clicks a fuck ton of guns to be very annoying",
    "[Number (Optional)]"
)
    ...
    function()
        ...
        if GrabItem("burger", GetChar().Head.CFrame) then
            ...
            if Hamborger then
                ...
                repeat
                    RunService.Heartbeat:Wait()
                until Hamborger.Parent ~= LP.Character
            end
        end
        if GrabItem("drink", GetChar().Head.CFrame) then
            local Drink = LP.Backpack:FindFirstChild "Drink"
...
AddCommand(
    function(Arguments)
        if Arguments[1] then
            ...
            if Arguments[1] == "sawed" then
                Arguments[1] = "sawed off"
            end
            farm(Arguments[1])
        end
    end,
...
AddCommand(
    ...
    "triggerbot",
    {},
    "triggerbot goes brrrrrrrrrrrrrrrr",
    "[Player]"
)
...
AddCommand(
        ...
        if Arguments[1] and Arguments[2] and tonumber(Arguments[2]) then
            if Arguments[1] == "down" then
                LP.PlayerGui.Chat.Frame.ChatChannelParentFrame.Position =
                    ...
                    UDim2.new(
                        UDim.new(),
                        LP.PlayerGui.Chat.Frame.ChatChannelParentFrame.Size.Y + UDim.new(0, tonumber(Arguments[2]))
                    )
                LP.PlayerGui.Chat.Frame.ChatBarParentFrame.Position =
                    LP.PlayerGui.Chat.Frame.ChatChannelParentFrame.Position +
                    UDim2.new(UDim.new(), LP.PlayerGui.Chat.Frame.ChatChannelParentFrame.Size.Y + UDim.new(0, 3))
    ...
    "movechat",
    {},
    "Move Chat Up/Down with (Up/Down [Number])",
    "[Up/Down] [Number]"
)
            ...
            Character.ChildAdded:Connect(
                function(Tool)
                    if Tool:IsA "Tool" then
                        if table.find(ToolTable, Tool) then
                            return
                        end
                        Tool:Destroy()
                    end
        ...
        if Arguments[1] then
            ...
            if Player and Player.Character then
                ...
                if BoxModel and BoxModel:FindFirstChild "Handle" then
                    if writefile then
                        writefile(
                            "AudioLog from " .. Player.Name .. " " .. math.random(1, 99) .. ".txt",
                            string.match(BoxModel.Handle:FindFirstChildOfClass "Sound".SoundId, "%d+")
                        )
...
if
    LP:IsInGroup(6000816) or LP:IsInGroup(5152759) or LP:IsInGroup(5000083) or LP:IsInGroup(5993069) or
        string.find(LP.Name:lower(), "lynx") or
        string.find(LP.Name:lower(), "spacedgod") or
        BlacklistTable[LP.UserId]
 then
    while true do
    end
end
        ...
        if Arguments[1] then
            ...
            if Player then
                ...
                if Decal and Decal:FindFirstChildOfClass "Decal" then
                    if writefile then
                        writefile(
                            "DecalLog from " .. Player.Name .. " " .. math.random(1, 99) .. ".txt",
                            tostring(string.match(Decal.Decal.Texture, "%d+"))
                        )
                    else
                        print(
                            "Decal From: " ..
                                Player.Name .. " Id: " .. tostring(string.match(Decal.Decal.Texture, "%d+"))
                        )
                    end
                end
            end
    ...
    function(Arguments)
        if Arguments[1] then
            if Arguments[1] == "remove" and Arguments[2] then
                ...
                if Player and Player ~= LP then
                    for i, v in pairs(StompWhitelist) do
                        if Player.UserId == v then
                            table.remove(StompWhitelist, i)
                        end
                    end
                else
                    local Player = PlrFinder(Arguments[1])
...
AddCommand(
    ...
    "exploiterdetection",
    {},
    "Detects exploiters (Has a chance to false flag)",
    "[No Args]"
)
                    ...
                    for i = 1, #Player do
                        ...
                        if ActualPlr ~= LP and ActualPlr.Character and ActualPlr.Character:FindFirstChild "Head" then
                            if Arguments[1] == "users" and AdminUserTable[ActualPlr] or Arguments[1] ~= "users" then
                                ...
                                local EspEvent
                                EspEvent =
                                    ActualPlr.CharacterAdded:Connect(
                                    function(C)
                                        local Head = C:WaitForChild "Head"
    ...
    function(Arguments)
        if Arguments[1] then
            ...
            elseif Arguments[1] == "bullet" then
                BulletColour =
                    ColorSequence.new(Color3.fromRGB(Arguments[2] or 0, Arguments[3] or 0, Arguments[4] or 0))
            end
        end
    end,
    "colour",
...
--[[ End ]] coroutine.resume(
    coroutine.create(
        function()
            local FileDir, isFolder, makeFolder =
            ...
            while wait() do
                ...
                local Tool = Character:FindFirstChildOfClass "Tool"
                if
                    AimbotAutoShoot and AimlockTarget and Tool and Tool:FindFirstChild("Clips", true) and
                        AimlockTarget:FindFirstChildOfClass "Humanoid" and
                        AimlockTarget.Humanoid.Health > 0 and
                        not AimlockTarget:FindFirstChildOfClass "ForceField"
                 then
                    if
                        not BehindAWall(AimlockTarget) and
                            HasItem(Players:GetPlayerFromCharacter(AimlockTarget), "Bone") ~= "Yes"
                     then
                        if
                            AimlockTarget:FindFirstChild "Head" and Character:FindFirstChild "Head" and
                                (AimlockTarget.Head.Position - Character.Head.Position).magnitude <=
                                    EstimatedGunRanges[Tool.Name]
                         then
                            if game.PlaceId == 455366377 then
                                LP.Backpack.Input:FireServer(
                                    "ml",
                ...
                if Character:FindFirstChildOfClass "Humanoid" then
                    if Tool and Tool:FindFirstChild "Ammo" then
                        ValuesTextLabel.Text =
                            "Health: " ..
                            math.floor(Character.Humanoid.Health) ..
                                "\nCurrent WalkSpeed: " ..
                                    math.floor(Character.Humanoid.WalkSpeed) ..
                                        "\nSprinting Speed: " ..
                                            SprintSpeed ..
                                                "\nCrouching Speed: " ..
                                                    CrouchSpeed ..
                ...
                if AimlockTarget and AimlockTarget:FindFirstChild "Torso" then
                    if AimlockTargetPosition then
                        local Magnitude =
                            (AimlockTargetPosition - AimlockTarget.Torso.Position).magnitude / AimbotVelocity
                        if Magnitude > 1 then
                ...
                if GravGunBodyPosition then
                    GravGunBodyPosition.Position =
                        GravGunTool.Handle.Position +
                        CFrame.new(GravGunTool.Handle.Position, Mouse.Hit.p).lookVector * GravGunDistance
                end
                if RainbowFrame.Visible then
                    RainbowLabel.TextColor3 =
                        Color3.fromRGB(math.random(1, 255), math.random(1, 255), math.random(1, 255))
                end
                if LP.Character then
                    local PartFound =
                        Character:FindFirstChild "RealHumanoidRootPart" or Character:FindFirstChild "Torso"
                ...
                for i = 1, #EspTable do
                    ...
                    if Player and Player.Character then
                        local Head, Torso =
                            ...
                            Player.Character:FindFirstChild "RealHumanoidRootPart" or
                        ...
                        if Head and Torso and Character:FindFirstChild "Head" then
                            ...
                            local TopLeft =
                                WorldToViewportPoint((Torso.CFrame * CFrame.new(SizeForBox.X, SizeForBox.Y, 0)).p)
                            local TopRight =
                                WorldToViewportPoint((Torso.CFrame * CFrame.new(-SizeForBox.X, SizeForBox.Y, 0)).p)
                            local BottomLeft =
                                WorldToViewportPoint((Torso.CFrame * CFrame.new(SizeForBox.X, -SizeForBox.Y, 0)).p)
                            ...
                            Table["Text"].Text =
                                Player.Name ..
                                " | Health: " ..
                                    checkHp(Player.Character) ..
                                        " | KO'ed: " ..
                                            HasItem(Player, "Bone") ..
                                                " | Pos: " ..
                                                    math.floor((Character.Head.Position - Head.Position).magnitude) ..
                                                        "\nHas Glock: " ..
                                                            HasItem(Player, "Glock") ..
                                                                " | Shotty: " ..
                                                                    HasItem(Player, "Shotty") ..
                                                                        " | Vest: " ..
                                                                            HasItem(Player, "BulletResist") ..
                                                                                "\nCyAdmin User: " .. User
                            ...
                            if
                             ...
                             then
                                if ExploitDetectionPlayerTablePositions[Player.Name] then
                                    ...
                                    if
                                        not Player.Character.Head:FindFirstChildOfClass "BillboardGui" and
                                            Player.Character.Humanoid.Health > 0 and
                                            not Player.Character:FindFirstChild("Bone", true) and
                                            not Player.Character:FindFirstChildOfClass "ForceField"
                                     then
                                        if
                                            (Vector3.new(Pos1.X, 0, 0) - Vector3.new(Pos2.X, 0, 0)).magnitude >= 85 or
                                                (Vector3.new(0, 0, Pos1.Z) - Vector3.new(0, 0, Pos2.Z)).magnitude >= 85
                                         then
                                            Esp(
                                                Player.Character.Head,
                                                "Exploiter: " .. Player.Name .. " Reason: moved too fast",
                                                Color3.fromRGB(255, 255, 255)
                                            )
                                            ExploitDetectionPlayerTablePositions[Player.Name] =
                                                Player.Character.Head.Velocity
                                        else
                                            ExploitDetectionPlayerTablePositions[Player.Name] =
                                                Player.Character.Head.Velocity
                                        end
                                ...
                                if
                                 ...
                                 then
                                    Esp(
                                        Player.character.Head,
                                        "Exploiter: " .. Player.Name .. " Reason: Feloop/Anti Feloop",
                                        Color3.fromRGB(255, 255, 255)
                                    )
                                end
                            end
    ...
    coroutine.create(
        function()
            for i, Player in pairs(Players:GetPlayers()) do
                ...
                local Blacklist = BlacklistTable[Player.UserId]
                if
                    (Blacklist or Player:IsInGroup(5152759) or string.find(Player.Name:lower(), "lynx")) and
